...
void CListenSocket::OnAccept(int nErrorCode)
{
    ...
    if (Accept(*pClient)) { // 클라이언트 접속 요청이 오면 서버-클라이언트를 연결시켜준다
        ...
        CSCREAMMINGSERVERDlg* pMain = (CSCREAMMINGSERVERDlg*)AfxGetMainWnd(); // CSocketServerDlg의 핸들을 가져옴
        str.Format(_T("Client (%d)"), (int)m_ptrClientSocketList.Find(pClient)); // 클라이언트 번호(POSITION(주소) 값)
        pMain->m_clientList->AddString(str); // 클라이언트가 접속할때마다 리스트에 클라이언트 이름 추가
...
void CListenSocket::CloseClientSocket(CSocket* pClient)
{
    ...
    if (pos != NULL) {
        ...
        CString str1, str2;
        UINT indx = 0, posNum;
        pMain->m_clientList->SetCurSel(0);
        // 접속 종료되는 클라이언트 찾기
        while (indx < pMain->m_clientList->GetCount()) {
            posNum = (int)m_ptrClientSocketList.Find(pClient);
            pMain->m_clientList->GetText(indx, str1);
            str2.Format(_T("%d"), posNum);
            // 소켓리스트, 클라이언트리스트를 비교해서 같은 클라이언트 번호(POSITION 값) 있으면 리스트에서 삭제
            if (str1.Find(str2) != -1) {
                AfxMessageBox(str1 + str2);
                pMain->m_clientList->DeleteString(indx);
                break;
            }
            indx++;
        }
